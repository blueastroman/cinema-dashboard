"""Refresh IFC Center showtimes in data.json without running the full weekly scrape."""
...
from scrape import fetch_ifc_showtimes  # noqa: E402
...
def unique_movie_id(title: str, ratings: dict[str, Any], used_ids: set[str]) -> str:
    base_id = make_movie_id(title, ratings) or "ifc-movie"
    movie_id = base_id
    suffix = 2
...
def merge_ifc_entries(dataset: dict[str, Any], entries: list[dict[str, Any]]) -> None:
...
def main() -> int:
    print(f"Refreshing {THEATER_NAME} showtimes...")
    theater = {"name": THEATER_NAME, **THEATER_CONFIG[THEATER_NAME]}
    entries = fetch_ifc_showtimes(theater)
    print(f"  Fetched {len(entries)} showtime entries")

    dataset = load_dataset(OUTPUT_DATA_PATH)
    merge_ifc_entries(dataset, entries)
    save_dataset(OUTPUT_DATA_PATH, dataset)

    ifc_movies = [m for m in dataset["movies"] if any(t["name"] == THEATER_NAME for t in m.get("theaters", []))]
    print(f"  {len(ifc_movies)} films at {THEATER_NAME} written to data.json")
    return 0
